...
const MEETING_PROMPT_CONTENT = [
  ...
  '- 개요는 짧게 쓰되 상세 본문에는 각 안건의 배경, 수치, 비교 대안, 결정 근거와 남은 쟁점을 보존하세요.',
].join('\n');
...
const LECTURE_PROMPT_CONTENT = [
  ...
  '- 긴 강의는 실제 설명 순서에 따라 모든 핵심 개념과 문제 풀이 단계, 구체적인 예시, 질문과 답변을 보존하세요.',
].join('\n');
...
const MENTORING_PROMPT_CONTENT = [
  ...
  '- 주제별 조언의 배경, 적용 조건, 사례, 주의할 점을 구체적으로 보존하고 임의의 주제 수로 압축하지 마세요.',
].join('\n');
